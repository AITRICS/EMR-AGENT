""" eICU Evaluation
This code reuses code used in the eICU Benchmarks.
[GitHub] https://github.com/mostafaalishahi/eICU_Benchmark

The code reuses only the necessary parts of tasks from the following source:
...
### [Reliability] ###
# Load the data
df_patient = dataframe_from_csv(paths['path_source_eicu'], 'patient', compress='gzip')
df_patient = generate_los(df_patient)
df_patient = generate_mortality(df_patient)
report_case(df_patient)

## --- [Exclusion criteria 1] age
df_exclusion1 = filter_patients_on_age(df_patient, min_age=18, max_age=89)
report_case(df_exclusion1, step='Exclusion 1: Adult patients')
save_to_csv(df_exclusion1, path=path_eval, filename='reliability_eicu_cohort1')

## --- [Exclusion criteria 2] icu stay
df_exclusion2 = filter_one_unit_stay(df_exclusion1)
report_case(df_exclusion2, step='Exclusion 2: Just one stay in ICU')
save_to_csv(df_exclusion2, path=path_eval, filename='reliability_eicu_cohort2')


### [Harmonizability] ###
# Laboratory & Feature Mapping data ###
print(f"\nLoading laboratory and feature mapping data...\n")
labs = dataframe_from_csv(paths['path_source_eicu'], 'lab', compress='gzip')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria1')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria2')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria3')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria4')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria5')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria6')
...
# Save the result
save_to_csv(patients, path=path_eval, filename='harmonizability_eicu_criteria7')
