...
# Import: DB connection utility (EMR-SQL_V10 style)
from utils import PostgreSQLConnector, schema_extract_query, foreignkey_extact_query, tab2col_dictionary_join, fk_dictionary_join
from utils import llm_api_generation, sql_post_process
from utils import *
...
Table Player_Attributes, columns = [*,id,player_fifa_api_id,player_api_id,date,overall_rating,potential,preferred_foot,attacking_work_rate,defensive_work_rate,crossing,finishing,heading_accuracy,short_passing,volleys,dribbling,curve,free_kick_accuracy,long_passing,ball_control,acceleration,slogging.info_speed,agility,reactions,balance,shot_power,jumping,stamina,strength,long_shots,aggression,interceptions,positioning,vision,penalties,marking,standing_tackle,sliding_tackle,gk_diving,gk_handling,gk_kicking,gk_positioning,gk_reflexes]
...
'''
classification_prompt = '''# For the given question, classify it as EASY, NON-NESTED, or NESTED based on nested queries
and JOIN.
...
easy_prompt = '''# Use the the schema links to generate the SQL queries for each of the questions.

{schema_information}

Q: "Find the buildings which have rooms with capacity more than 50."
Schema_links: [classroom.building,classroom.capacity,50]
SQL: SELECT DISTINCT building FROM DB_NAME.classroom WHERE capacity  >  50

Q: "Find the room number of the rooms which can sit 50 to 100 students and their buildings."
Schema_links: [classroom.building,classroom.room_number,classroom.capacity,50,100]
SQL: SELECT building ,  room_number FROM DB_NAME.classroom WHERE capacity BETWEEN 50 AND 100
...
SQL: SELECT name FROM DB_NAME.student WHERE dept_name  =  'History' ORDER BY tot_cred DESC LIMIT 1
...
SQL: SELECT sum(budget) FROM DB_NAME.department WHERE dept_name  =  'Marketing' OR dept_name  =  'Finance'
...
SQL: SELECT dept_name FROM DB_NAME.instructor WHERE name LIKE '%Soisalon%'

Q: "What is the name of the department with the most credits?"
Schema_links: [course.dept_name,course.credits]
SQL: SELECT dept_name FROM DB_NAME.course GROUP BY dept_name ORDER BY sum(credits) DESC LIMIT 1
...
SQL: SELECT COUNT (DISTINCT ID) FROM DB_NAME.teaches WHERE semester  =  'Spring' AND YEAR  =  2010
...
SQL: SELECT name ,  dept_name FROM DB_NAME.student ORDER BY tot_cred

Q: "Find the year which offers the largest number of courses."
Schema_links: [SECTION.YEAR,SECTION.*]
SQL: SELECT YEAR FROM DB_NAME.SECTION GROUP BY YEAR ORDER BY count(*) DESC LIMIT 1
...
SQL: SELECT dept_name ,  AVG (salary) FROM DB_NAME.instructor GROUP BY dept_name HAVING AVG (salary)  >  42000
...
SQL: SELECT count(*) ,  building FROM DB_NAME.classroom WHERE capacity  >  50 GROUP BY building

Q: "Find the names of the top 3 departments that provide the largest amount of courses?"
Schema_links: [course.dept_name,course.*]
SQL: SELECT dept_name FROM DB_NAME.course GROUP BY dept_name ORDER BY count(*) DESC LIMIT 3

Q: "Find the maximum and average capacity among rooms in each building."
Schema_links: [classroom.building,classroom.capacity]
SQL: SELECT max(capacity) ,  avg(capacity) ,  building FROM DB_NAME.classroom GROUP BY building

Q: "Find the title of the course that is offered by more than one department."
Schema_links: [course.title]
SQL: SELECT title FROM DB_NAME.course GROUP BY title HAVING count(*)  >  1

'''
medium_prompt = '''# Use the the schema links and Intermediate_representation to generate the SQL queries for each of
the questions.
...
SQL: SELECT dept_name ,  building FROM DB_NAME.department ORDER BY budget DESC LIMIT 1
...
SQL: SELECT dept_name ,  building FROM DB_NAME.department WHERE budget  >  (SELECT avg(budget) FROM DB_NAME.department)
...
SQL: SELECT count(DISTINCT T2.id) ,  count(DISTINCT T3.id) ,  T3.dept_name FROM DB_NAME.department AS T1 JOIN DB_NAME.student AS T2 ON T1.dept_name  =  T2.dept_name JOIN DB_NAME.instructor AS T3 ON T1.dept_name  =  T3.dept_name GROUP BY T3.dept_name
...
SQL: SELECT T1.title FROM DB_NAME.course AS T1 JOIN DB_NAME.prereq AS T2 ON T1.course_id  =  T2.course_id GROUP BY T2.course_id HAVING count(*)  =  2
...
SQL: SELECT DISTINCT T1.name FROM DB_NAME.student AS T1 JOIN DB_NAME.takes AS T2 ON T1.id  =  T2.id WHERE T2.YEAR  =  2009 OR T2.YEAR  =  2010
...
SQL: SELECT T1.title ,  T3.name FROM DB_NAME.course AS T1 JOIN DB_NAME.teaches AS T2 ON T1.course_id  =  T2.course_id JOIN DB_NAME.instructor AS T3 ON T2.id  =  T3.id WHERE T2.YEAR  =  2008 ORDER BY T1.title

'''
hard_prompt = '''# Use the intermediate representation and the schema links to generate the SQL queries for each of
the questions.
...
SQL: SELECT T1.title ,  T1.credits , T1.dept_name FROM DB_NAME.course AS T1 JOIN DB_NAME.prereq AS T2 ON T1.course_id  =  T2.course_id GROUP BY T2.course_id HAVING count(*)  >  1
...
SQL: SELECT id FROM DB_NAME.teaches WHERE semester  =  'Fall' AND YEAR  =  2009 EXCEPT SELECT id FROM DB_NAME.teaches WHERE semester  =  'Spring' AND YEAR  =  2010
...
SQL: SELECT title FROM DB_NAME.course WHERE course_id NOT IN (SELECT course_id FROM DB_NAME.prereq)
...
SQL: SELECT DISTINCT salary FROM DB_NAME.instructor WHERE salary  <  (SELECT max(salary) FROM DB_NAME.instructor)
...
SQL: SELECT name FROM DB_NAME.student WHERE id IN (SELECT id FROM DB_NAME.takes WHERE semester  =  'Fall' AND YEAR  =  2003)
...
SQL: SELECT min(salary) ,  dept_name FROM DB_NAME.instructor GROUP BY dept_name HAVING avg(salary)  >  (SELECT avg(salary) FROM DB_NAME.instructor)
...
SQL: SELECT title FROM DB_NAME.course WHERE course_id IN (SELECT T1.prereq_id FROM DB_NAME.prereq AS T1 JOIN DB_NAME.course AS T2 ON T1.course_id  =  T2.course_id WHERE T2.title  =  'Mobile Computing')
...
SQL: SELECT T3.title ,  T3.credits FROM DB_NAME.classroom AS T1 JOIN DB_NAME.SECTION AS T2 ON T1.building  =  T2.building AND T1.room_number  =  T2.room_number JOIN DB_NAME.course AS T3 ON T2.course_id  =  T3.course_id WHERE T1.capacity  =  (SELECT max(capacity) FROM DB_NAME.classroom)
"
'''
#----------------------------------------------------------------------------------------------------------
def setup_logging(results_dir):
    results_dir = results_dir + f"bs_dinsql_cohort/DINSQL_{args.case_name}_DB{args.target_database}_Pior{args.database_knowledge}_LLM{args.llm_model}"
    if os.path.exists(results_dir):
        shutil.rmtree(results_dir)
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    log_filename = f"{results_dir}/DINSQL_log.txt"

    logging.basicConfig(
        level=logging.INFO,
        format='%(message)s',
        handlers=[
            logging.FileHandler(log_filename),  # This will also logging.info to console
    ...
    return results_dir
#----------------------------------------------------------------------------------------------------------
def load_schema_foreignkey(db_connector):
    schema_link = db_connector.connect(schema_extract_query)
    schema_link = tab2col_dictionary_join(schema_link, ['schema', 'table', 'column'])
    table_names = list(set([i.split('.')[0] for i in schema_link]))
    foreign_key_list = []
    for schema_name in table_names:
        fks = db_connector.connect(foreignkey_extact_query.replace("Schema_Name", schema_name))
        if fks:
            foreign_key_list.extend(fk_dictionary_join(fks))
    db_schema_lines = schema_link
    foreign_keys = foreign_key_list
    return db_schema_lines, foreign_keys

# ----------------------- Schema Linking Prompt Generation -----------------------
def generate_schema_linking_prompt(db_connector):
    db_schema_lines, foreign_keys = load_schema_foreignkey(db_connector)
    schema_prompt_lines = []
    for line in db_schema_lines:
        ...
        if table_match:
            ...
            schema_prompt_lines.append(f"Table {table}, columns = [*,{columns}]")
    foreign_key_prompt = "Foreign_keys = [" + ",".join(foreign_keys) + "]"
    schema_linking_prompt_final = schema_linking_prompt + "\n" + "\n".join(schema_prompt_lines) + "\n" + foreign_key_prompt
    return schema_linking_prompt_final

# ----------------------- Prompt Makers -----------------------
def schema_linking_prompt_maker(question, schema_prompt):
    return schema_prompt + f'\nQ: "{question}"\nA: Let\'s think step by step.'

def classification_prompt_maker(question, schema_prompt, schema_links):
    return classification_prompt.replace('{schema_information}',schema_prompt) + f'\nQ: "{question}"\nschema_links: {schema_links}\nA: Let\'s think step by step.'

def easy_prompt_maker(question, schema_prompt, schema_links):
    return easy_prompt.replace('{schema_information}',schema_prompt) + f'\nQ: "{question}"\nSchema_links: {schema_links}\nSQL:'

def medium_prompt_maker(question, schema_prompt, schema_links):
    return medium_prompt.replace('{schema_information}',schema_prompt) + f'\nQ: "{question}"\nSchema_links: {schema_links}\nA: Let\'s think step by step.'

def hard_prompt_maker(question, schema_prompt, schema_links, sub_question):
    return hard_prompt.replace('{schema_information}',schema_prompt) + f'\nQ: "{question}"\nSchema_links: {schema_links}\nA: Let\'s think step by step. "{question}" can be solved by knowing the answer to the following sub-question "{sub_question}".'

def self_correction_prompt_maker(question, sql_query, schema_prompt):
    return f"""#### For the given question, use the provided tables, columns, foreign keys to fix the SQL. If correct, return as is.\nSchema: {schema_prompt}\nQuestion: {question}\nSQL Query: {sql_query}\n#### Fixed SQL Query:\nSELECT"""

def cohort_feature_agent(args, results_dir, schema_linking_prompt_final, trial_index):
    question = f"List all {args.requested_features} information that satisfy following [Condition].\n[Condition]: {args.cohort_selection}\nMake PostgreSQL follow the order of the provided information, categories, type."

    # Step 1: Schema Linking
    linked_schema = llm_api_generation(schema_linking_prompt_maker(question, schema_linking_prompt_final))
    logging.info("\n[Schema Linking Result]")
    logging.info(linked_schema)

    # Step 2: Classification
    classification = llm_api_generation(classification_prompt_maker(question, schema_linking_prompt_final, linked_schema))
    logging.info(f"\n[Difficulty Classification]: {classification}")

    # Step 3: SQL Generation
    if '"EASY"' in classification or 'EASY' in classification:
        sql_query = llm_api_generation(easy_prompt_maker(question, schema_linking_prompt_final, linked_schema))
    elif '"NON-NESTED"' in classification or 'NON-NESTED' in classification:
        sql_query = llm_api_generation(medium_prompt_maker(question, schema_linking_prompt_final, linked_schema))
    else:
        sub_question = ""  # Optional: Extract sub-question from classification if necessary
        sql_query = llm_api_generation(hard_prompt_maker(question, schema_linking_prompt_final, linked_schema, sub_question))

    logging.info("\n[Initial Generated SQL Query]")
    logging.info(sql_query)

    # Step 4: Self Correction
    corrected_sql = llm_api_generation(self_correction_prompt_maker(question, sql_query, schema_linking_prompt_final))
    corrected_sql = sql_post_process(corrected_sql)
    logging.info("\n[Final Self-Corrected SQL Query]")
    logging.info(corrected_sql)
    
    try:
        final_output = db_connector.connect(corrected_sql)
        logging.info("\n[Final Output]")
        logging.info(final_output[:2])
    ...
    final_result = {
        "cohort_feature_sql" : corrected_sql,
        "cohort_feature": final_output,
...
if __name__ == '__main__':    
    ...
    parser.add_argument('--case-name', type=str, default="test1")
    parser.add_argument('--results-dir', type=str, default="./results/")
    parser.add_argument('--database-knowledge', type=str, default="db_and_manual_prior", choices = ["parametric_knowledge", "db_only", "db_and_manual", "db_and_manual_and_prior"])
    ...
    if "prior" in database_knowledge:
        if agent_part == "cohort":
            with open(f"./models/data/db_info/{target_database}_prior_knowledge_cohort.txt", 'r', encoding="UTF-8") as file:
                knowledge_source += ("DB-specific knowledge: " + file.read())
