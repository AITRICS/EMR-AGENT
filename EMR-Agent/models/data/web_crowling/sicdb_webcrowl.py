...
def extract_text_from_link(base_url, link_url):
    ...
    try:
        ...
        navigatemenu_info = main_content.find_all('div', id="mw-navigation")
        if navigatemenu_info:
            for navigatemenu in navigatemenu_info:
                navigatemenu.decompose()
        
        search_info = main_content.find_all('div', role="search")
        if search_info:
            for search in search_info:
                search.decompose()

        jump_info = main_content.find_all('a', class_="mw-jump-link")
        if jump_info:
        ...
        navigate_info = main_content.find_all('nav', role='navigation')
        if navigate_info:
            for navigate in navigate_info:
                navigate.decompose()
        
        footer_info = main_content.find_all('footer')
        if footer_info:
...
def scrape_mimic_tables_info(url):
    ...
    headers = main_content.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'li'])
    for header in headers:
        ...
        while next_element and next_element.name not in ['h1', 'h2', 'h3', 'h4', 'li']:
...
def save_to_txt(df, filename="sicdb_tables_info.txt"):
    """Save results to a TXT file."""
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("=== SICDB Tables Information ===\n\n")
...
if __name__ == "__main__":
    ...
    url = "https://www.sicdb.com/Documentation/Main_Page"
    try:
